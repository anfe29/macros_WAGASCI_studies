...
def plot_xsec_errs(file, subsets, dials, idials, pot, fit_paths_fgd_fhc):
    ...
    for i in range(len(dials)):
        gr_temp = TGraph(len(pot), apot, rel_error_evolution(i, idials, pot, fit_paths_fgd_fhc, xsectype))
        ...
        gr_temp2 = TGraph(len(pot), apot, rel_error_evolution(i, idials, pot, fit_paths_sfgd_fhc, xsectype))
        ...
        gr_temp3 = TGraph(len(pot), apot, rel_error_evolution(i, idials, pot, fit_paths_sfgd_wagasci_fhc, xsectype))
        #gr_temp3.SetTitle(dials[i])
    ...
    relativeUncertainties1.SetMaximum(relativeUncertainties1.GetYaxis().GetXmax()*1.1)
    relativeUncertainties1.SetMinimum(relativeUncertainties3.GetYaxis().GetXmin()*0.9)
    ...
    c1.SaveAs("plots/pot_studies/xsec/onlyFHC_withSK/{}_migrad.png".format(subsets))
    file.cd()
...
file = TFile("plots/pot_studies/xsec/onlyFHC_withSK/plots.root","RECREATE")
