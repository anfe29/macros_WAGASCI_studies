...
//FHC WAGASCI WG #nu_{#mu} CC 1#pi 
    ...
    std::vector<std::vector<std::vector<double>>> binpmu = {
        {   //FHC WAGASCI PM-WMRD #nu_{#mu} CC 0#pi 
            ...
            {0, 450, 500, 550, 600, 650, 30000},
            {0, 450, 500, 550, 600, 650, 700, 30000},
            {0, 450, 500, 550, 600, 650, 700, 30000},
            {0, 450, 500, 550, 600, 650, 700, 750, 800, 30000},
            {0, 800, 900, 1100, 30000}
        }, 
        {   //FHC WAGASCI PM-BM #nu_{#mu} CC 0#pi
            {0, 600, 650, 700, 800, 900, 30000},
            {0, 600, 650, 700, 800, 900, 1200, 30000},
        ...
        {   //FHC WAGASCI PM #nu_{#mu} CC 1#pi 
            ...
            {0, 400, 500, 550, 600, 650, 30000},
            {0, 400, 500, 550, 600, 650, 700, 30000},
            {0, 400, 500, 550, 600, 650, 700, 800, 900, 30000},
            {0, 500, 550, 600, 650, 700, 800, 900, 1400, 1700, 30000},
            {0, 500, 550, 600, 650, 700, 800, 900, 1400, 1700, 30000}
        },
        {   //FHC WAGASCI UWG-WMRD #nu_{#mu} CC 0#pi 
            ...
            {0, 400, 450, 500, 550, 600, 650, 700, 30000},
            {0, 400, 450, 500, 550, 600, 650, 700, 30000},
            {0, 450, 500, 550, 600, 650, 700, 800, 30000},
            {0, 500, 550, 600, 650, 700, 800, 900, 1100, 30000}
        },
        {   //FHC WAGASCI UWG-BM #nu_{#mu} CC 0#pi
            {0, 600, 650, 700, 750, 800, 900, 1100, 30000},
            {0, 650, 700, 750, 800, 900, 1100, 1400, 30000},
            {0, 650, 700, 750, 800, 900, 1100, 1400, 1800, 30000},
            {0, 650, 700, 750, 800, 900, 1100, 1400, 1800, 30000},
            {0, 650, 700, 750, 800, 900, 1100, 1400, 1800, 30000}
        },
        {   //FHC WAGASCI DWG-BM #nu_{#mu} CC 0#pi 
        ...
        {   //FHC WAGASCI WG #nu_{#mu} CC 1#pi
            ...
            {0, 500, 550, 600, 650, 700, 750, 800, 900, 1100, 1400, 1700, 30000},
            {0, 400, 500, 550, 600, 650, 700, 750, 800, 900, 1100, 1400, 1700, 30000},
            {0, 400, 500, 550, 600, 650, 700, 750, 800, 900, 1100, 1400, 1700, 30000},
            {0, 400, 500, 550, 600, 650, 700, 750, 800, 900, 1100, 1400, 30000},
            {0, 400, 500, 550, 600, 650, 700, 750, 800, 900, 1100, 1400, 1700, 30000},
            {0, 400, 500, 550, 600, 650, 700, 750, 800, 900, 1100, 1400, 1700, 30000}
        },
    };
    ...
    for(int isample = 0; isample < SAMPLES; isample++) {
        ...
        hpmu_res[isample].resize(bincos[isample].size());
        // create hist for resolution and pmu per bin, then initialize them
       for(int i = 0; i < bincos[isample].size()-1; i++) {
            hpmu[isample].push_back(new TH1D("", "", binpmu[isample][i].size()-1, &binpmu[isample][i][0]));
            hcs_res.push_back(new TH1D("", "", 100 , -1, 1));
            for(int j = 0; j < binpmu[isample][i].size()-1; j++) {
                hpmu_res[isample][i].push_back(new TH1D("", "", 100 , -1, 1));
            }
        }
    }
    ...
    for(int isample = 0; isample < SAMPLES; isample++) {
        ...
        for(int ibin = 1; ibin < hcs[isample]->GetNbinsX() + 1; ibin++) {
            ...
            tmp = "res_cos_bin"+to_string(ibin);
            hcs_res[ibin-1]->SetName(tmp.c_str());
            tmp = "Resolution cos #theta_{#mu} for bin "+to_string(ibin);
            hcs_res[ibin-1]->SetTitle(tmp.c_str());
            hcs_res[ibin-1]->GetXaxis()->SetTitle("#frac{CosThetamu - TrueCosThetamu}{TrueCosThetamu}");
            hcs_res[ibin-1]->GetYaxis()->SetTitle("Events");
            hcs_res[ibin-1]->SetOption("HISTE");
            hcs_res[ibin-1]->SetFillColor(kRed);
            hcs_res[ibin-1]->SetFillStyle(3004);
            subc->cd();
            hcs_res[ibin-1]->Write();

            tmp = "samp_pmu_cos_bin"+to_string(ibin);
            hpmu[isample][ibin-1]->SetName(tmp.c_str());
            tmp = "Distribution P_{#mu} for cos bin"+to_string(ibin)+" "+nhist[isample];
            hpmu[isample][ibin-1]->SetTitle(tmp.c_str());
            hpmu[isample][ibin-1]->GetXaxis()->SetTitle("P_{#mu}");
            hpmu[isample][ibin-1]->GetYaxis()->SetTitle("Events");
            hpmu[isample][ibin-1]->SetOption("HISTE");
            hpmu[isample][ibin-1]->SetFillColor(kGreen);
            hpmu[isample][ibin-1]->SetFillStyle(3004);
            subb->cd();
            hpmu[isample][ibin-1]->Write();
            ...
            for(int jbin = 1; jbin < hpmu[isample][ibin-1]->GetNbinsX() + 1; jbin++) {
                ...
                tmp = "res_pmu_bin"+to_string(ibin)+"_"+to_string(jbin);
                hpmu_res[isample][ibin-1][jbin-1]->SetName(tmp.c_str());
                tmp = "Resolution P_{#mu} for cos bin"+to_string(ibin)+" and pmu bin"+to_string(jbin);
                hpmu_res[isample][ibin-1][jbin-1]->SetTitle(tmp.c_str());
                hpmu_res[isample][ibin-1][jbin-1]->GetXaxis()->SetTitle("#frac{Pmu - TruePmu}{TruePmu}");
                hpmu_res[isample][ibin-1][jbin-1]->GetYaxis()->SetTitle("Events");
                hpmu_res[isample][ibin-1][jbin-1]->SetOption("HISTE");
                hpmu_res[isample][ibin-1][jbin-1]->SetFillColor(kViolet);
                hpmu_res[isample][ibin-1][jbin-1]->SetFillStyle(3004);
                subd->cd();
                hpmu_res[isample][ibin-1][jbin-1]->Write();
